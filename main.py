...
Greeting card
Produce bags
Coupon for phone repair services
Wireless charger
Phone repair kit
Matcha
Match bamboo whisk
hemp Rolling papers
chargeable arc lighters
wood grinder""".split("\n")
print(t)
...
def main():
    def conduct_exhaustive_search(item_description):
        ...
        listing_urls_to_db(product_links, item_description)
...
def gather_product_listings(search_query):
    ...
    for page_number in range(1,99):
        ...
        try:
            url = \
                f"https://www.alibaba.com/trade/search?spm=a2700.galleryofferlist.0.0.63f05734cVVG1H&fsb=y&IndexArea=product_en&assessmentCompany=true&keywords={search_query}&ta=y&tab=all&viewType=L&&page={page_number}"
            ...
            for url in re.finditer('productUrl', resp):
                ...
                print(f"{len(listings)} link listing gathered")
...
def listing_urls_to_db(product_listings, item_description):
    ...
    for product_link in product_listings:
        ...
        chars = {"URL": product_link, "Search query used": item_description,'date-of-scraping': str(date.today())}
        for i in re.finditer('attrName', resp):
            i = i.start()
            key_char = resp[i:i + 60].split('"')[2]

            if 'attrVal' in key_char:
                pass
            else:
                print(key_char)
                print(resp[i:i + 200].split('attrValue')[1].split('"'))
                chars[key_char] = resp[i:i + 200].split('attrValue')[1].split('"')[2]
        ...
        for i in list(re.finditer('formatPrice', resp))[0:-2]:
            ...
            noisy_char = resp[i + 22:i + 65]
            dbug.append(noisy_char)
            for schar in ['}', '{', ':', ',',']','[']:
        ...
        for price_points_noisy in noisy_price_infos:
            ...
            for i in range(len(price_points_noisy)):
                ...
                if i % 2 == 0:
                    try:
                        price_points_noisy = list(filter(None,price_points_noisy))
                        k, v = price_points_noisy[i], price_points_noisy[i + 1]
                        paired_info[k] = v.replace('-1', '1000000')
                    ...
                    #    print(e)
                      #  print(price_points_noisy)
                        pass
            try:
                if 'min' in paired_info:
                    price_points[paired_info['price']] = [int(paired_info['min']), int(paired_info['max'])]
                    if count == 0:
                        count += 1
                        chars['minimum order qty'] = int(paired_info['min'])
                        print(paired_info['min'])
                        print(f"{count}: COUNT")
            except:
                continue
        ...
        try:
            ...
            print(f" Item_ID: {str(x.inserted_id)} was saved in db")
        except Exception as e:
            print(
